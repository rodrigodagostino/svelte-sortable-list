...
- An ancestor that creates a stacking context (`transform`, `filter`, `opacity` below `1`, `isolation: isolate`, `contain`, `will-change`, a positioned element with a `z-index`…) prevents the dragged item from being rendered above content outside of it.
...
- The scroll container is taken to be the nearest ancestor with `overflow: auto` or `overflow: scroll`, or the nearest dialog. Wrap the list so that this ancestor is the element that really scrolls, otherwise auto scrolling stops at an ancestor that never moves and the window never takes over.
- On touch devices, a tap on plain item content does not fire a `click` event, since the long-press context menu has to be suppressed there. Attach click handlers to interactive elements inside the item rather than to the item content itself.
...
The default is generated from the `direction` and `group` props, so it names the arrow keys that actually apply. For a vertical list without a group it reads: `"Press Up Arrow or Down Arrow to move through the list items. Press Space to start dragging an item. When dragging, use Up Arrow or Down Arrow to move the item around. Press Space again to drop the item, or Escape to cancel."` A list belonging to a `group` also describes the cross axis arrow keys, which move the focus to a different list and carry the dragged item across.
...
<SortableList.Root {direction} aria-description={ariaDescription} {announcements}>
	{#each items as item, index (item.id)}
		<SortableList.Item {...item} {index}>
			<div class="ssl-item-content">
				<span class="ssl-item-content__text">{item.text}</span>
...
| `<SortableList.ItemRemove>` | A `<button>` element that removes an item when pressed. Including this inside a `<SortableList.Item>` moves the focus to a neighboring item before calling your `onclick` handler, which handles the removal. |
...
| `delay`              | `number \| undefined`                   | `0`                                                         | Number ≥ `0`                                          | Time before the drag operation starts (in milliseconds). Can help prevent accidental dragging.                                                                                                                                                                                                                                                                       |
...
| `canClearOnDragOut`  | `boolean \| undefined`                  | `false`                                                     | `true` \| `false`                                     | When `true`, clears the target item when a dragged item (via pointing device) is dragged outside the list bounds and doesn't collide with any list items. This causes the dragged item to return to its initial position when dropped, rather than taking the position of the last item it collided with.                                                            |
| `canRemoveOnDropOut` | `boolean \| undefined`                  | `false`                                                     | `true` \| `false`                                     | When `true`, removes items that are dragged and dropped outside list boundaries. Should be used with the `ondrop` event handler and call `removeItem()` to complete the removal process.                                                                                                                                                                             |
...
| `announcements`      | `object \| undefined`                   | `undefined`                                                 | Object                                                | Custom announcements for screen readers during drag-and-drop operations.                                                                                                                                                                                                                                                                                             |
...
| `ondrag`      | `SortableList.RootEvents['ondrag']`      | Dragged item is moved by pointer device or keyboard (fires once per animation frame while the pointer moves, and once per key press) | <pre>{<br>&nbsp;&nbsp;deviceType: 'pointer' \| 'keyboard',<br>&nbsp;&nbsp;sourceList: HTMLUListElement,<br>&nbsp;&nbsp;sourceListId?: string,<br>&nbsp;&nbsp;sourceListIndex?: number,<br>&nbsp;&nbsp;draggedItem: HTMLLIElement,<br>&nbsp;&nbsp;draggedItemId: string,<br>&nbsp;&nbsp;draggedItemIndex: number,<br>&nbsp;&nbsp;targetList: HTMLUListElement \| null,<br>&nbsp;&nbsp;targetListId: string \| null,<br>&nbsp;&nbsp;targetListIndex: number \| null,<br>&nbsp;&nbsp;targetItem: HTMLLIElement \| null,<br>&nbsp;&nbsp;targetItemId: string \| null,<br>&nbsp;&nbsp;targetItemIndex: number \| null,<br>&nbsp;&nbsp;isWithinBounds: boolean,<br>&nbsp;&nbsp;canRemoveOnDropOut: boolean<br>}</pre>                                     |
| `ondrop`      | `SortableList.RootEvents['ondrop']`      | Dragged item is released by pointer device or keyboard                                                                               | <pre>{<br>&nbsp;&nbsp;deviceType: 'pointer' \| 'keyboard',<br>&nbsp;&nbsp;sourceList: HTMLUListElement,<br>&nbsp;&nbsp;sourceListId?: string,<br>&nbsp;&nbsp;sourceListIndex?: number,<br>&nbsp;&nbsp;draggedItem: HTMLLIElement,<br>&nbsp;&nbsp;draggedItemId: string,<br>&nbsp;&nbsp;draggedItemIndex: number,<br>&nbsp;&nbsp;targetList: HTMLUListElement \| null,<br>&nbsp;&nbsp;targetListId: string \| null,<br>&nbsp;&nbsp;targetListIndex: number \| null,<br>&nbsp;&nbsp;targetItem: HTMLLIElement \| null,<br>&nbsp;&nbsp;targetItemId: string \| null,<br>&nbsp;&nbsp;targetItemIndex: number \| null,<br>&nbsp;&nbsp;isWithinBounds: boolean,<br>&nbsp;&nbsp;canRemoveOnDropOut: boolean,<br>&nbsp;&nbsp;isCanceled: boolean<br>}</pre> |
...
| `.ssl-item[data-drag-state="ptr-predrop"]`           | Item settling into its dropped position.                                                                           |
...
| `.ssl-item[data-is-within-bounds="false"]`           | Item positioned outside list boundaries.                                                                           |
...
| `.ssl-placeholder`                                   | Placeholder marking the dragged item's vacated slot during a drag. In a target list, it marks the drop slot.       |
...
| `.ssl-placeholder[data-drag-state="ptr-remove"]`     | Placeholder present while the dragged item is dropped outside list boundaries (with `canRemoveOnDropOut` enabled). |
| `.ssl-placeholder[data-drag-state="kbd-drag-start"]` | Placeholder present as a keyboard drag starts.                                                                     |
| `.ssl-placeholder[data-drag-state="kbd-drag"]`       | Placeholder present during a keyboard drag.                                                                        |
| `.ssl-placeholder[data-drag-state="kbd-drop"]`       | Placeholder present during a keyboard drop.                                                                        |
| `.ssl-placeholder[data-drag-state="kbd-cancel"]`     | Placeholder present during a canceled keyboard drag.                                                               |
