...
import { getDisabledItems } from '../src/routes/fixtures.js';

test.describe('Sortable List - Disabled List', () => {
	test.beforeEach(async ({ page }) => {
		// Navigate to the Disabled List page
		await page.goto('/disabled-list');
	...
	test('should let the page scroll when swiping over item content', async ({ page, hasTouch }) => {
		...
		expect(initialItems).toEqual(getDisabledItems(5).map((item) => item.text));
		...
		// Verify the disabled list and its items leave touch gestures alone
		await expect(root).toHaveCSS('touch-action', 'auto');
		await expect(root.locator('[data-item-id="disabled-item-2"]')).toHaveCSS(
			'touch-action',
			'auto'
		);
		...
		// Find the text of Disabled Item 2
		const itemText = root.locator('[data-item-id="disabled-item-2"] .ssl-item-content__text');
		const textBox = await itemText.boundingBox();
		if (!textBox) throw new Error('Could not get Disabled Item 2 text bounding box');
