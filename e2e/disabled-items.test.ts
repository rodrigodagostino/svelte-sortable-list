...
test.describe('Sortable List - Disabled Items', () => {
	...
	test('should let the page scroll when swiping over a disabled item', async ({
		page,
		hasTouch,
	}) => {
		...
		await expect(root.locator('[data-item-id="disabled-item-1"]')).toHaveCSS(
			'touch-action',
			'auto'
		);
		...
		// Find the text of Disabled Item 1, then swipe up over it
		const disabledText = root.locator('[data-item-id="disabled-item-1"] .ssl-item-content__text');
		const disabledBox = await disabledText.boundingBox();
		if (!disabledBox) throw new Error('Could not get Disabled Item 1 text bounding box');

		const swipeFinger = {
			x: disabledBox.x + disabledBox.width / 2,
			y: disabledBox.y + disabledBox.height / 2,
			id: 1,
		};
